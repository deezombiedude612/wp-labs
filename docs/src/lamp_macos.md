...
To start the built-in Apache server, open the Terminal app from your Application folder or type **Terminal** in the Spotlight Search (shortcut: cmd '⌘' + Space Bar).
...
8.  Press control (⌃) + x and press Enter to exit the nano editor.
...
5.  Press control (⌃) + w, type in `LoadModule authz_core_module` and press Enter.
...
8.  Press control (⌃) + w, search up `DocumentRoot` and press Enter.
...
9.  Press control (⌃) + w, search up `AllowOverride None` and press Enter.
...
10. For extra measure, we can set the Apache Server to serve an index.php file instance first before an index.html.
    Press control (⌃) + w, search up `DirectoryIndex` and press Enter.
...
12. Press control (⌃) + x and press Enter to exit the nano editor.
...
6.  Press control (⌃) + o and press Enter to save the file.

7.  Press control (⌃) + x and press Enter to exit the nano editor.
...
4.  Open a new tab in the Terminal (or reboot the Terminal application).
    ...
    ::: tip NOTE ABOUT THIS TERMINAL WINDOW INTERFACE
...
8.  Press control (⌃) + w, search up `LoadModule php` and press Enter.
...
9.  Press control (⌃) + o and press Enter to save the file.

10. Press control (⌃) + x and press Enter to exit the nano editor.
...
This step will involve installing MySQL's Community Server inside your Mac to use – if you plan on learning more on MySQL databases, consider this a way to kill two birds with one stone! :grin:
...
1. Head over to the following link to download phpMyAdmin (which is in a zip file):
